...
public class MainActivity extends Activity implements ConnectionAPI.MessageLisner, View.OnClickListener{
				...
				alertDialogBuilder.setPositiveButton("はい",new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						...
						if(!(ip_list.equals("NONE"))){
							...
							Toast.makeText(MainActivity.this, "部屋「"+Item.name+"」に入れません", Toast.LENGTH_SHORT).show();
						}
					}
				});
				// Negativeボタンとリスナを設定
	...
	public void getMessage(String data) {
		...
		case 2:
			...
			handler.post(new Runnable(){
				public void run(){
					Toast.makeText(MainActivity.this,"部屋「"+message+"」から退出しました",Toast.LENGTH_SHORT).show();
				}
			});
			break;
		case 3: 
		...
		case 5:
			handler.post(new Runnable() {
				public void run() {
					if(message.equals("0")){
						Toast.makeText(MainActivity.this, "部屋を削除しました", Toast.LENGTH_LONG).show();
					}
					else if(message.equals("1")){
						Toast.makeText(MainActivity.this,"部屋は使用中のため削除不可",Toast.LENGTH_SHORT).show();
					}
				}
			});
			break;
	...
	public void showIsConnect(final boolean state){
		handler.post(new Runnable(){
			public void run(){
				if(state){
					Toast.makeText(MainActivity.this,"接続完了",Toast.LENGTH_SHORT).show();
				}
				else{
					Toast.makeText(MainActivity.this,"接続失敗",Toast.LENGTH_SHORT).show();
				}
			}
		});
	}
		...
		if(v == addRoom){
			...
			alertDialogBuilder.setPositiveButton("OK",new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					Toast.makeText(MainActivity.this, "部屋「"+editText.getText().toString()+"」を作成しました", Toast.LENGTH_SHORT).show();
				}
			});
			//ダイアログの表示
			alertDialogBuilder.create().show();
			...
			handler.post(new Runnable(){
				public void run(){
					Toast.makeText(MainActivity.this, "部屋情報を更新しました", Toast.LENGTH_SHORT).show();
				}
			});
		}
	}
