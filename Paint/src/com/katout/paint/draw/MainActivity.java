...
public class MainActivity extends Activity implements PaintView.MenuLiner{
	...
	public void paintMenuPos(int h, int y, boolean animation) {
		if(y > 20){
			...
			if(visible == View.GONE || paint_menu_b.getVisibility() == View.INVISIBLE){
				paint_menu_b.setVisibility(View.INVISIBLE);
				paint_menu_t.setVisibility(View.VISIBLE);
			}
			paint_menu_t.layout(paint_menu_t.getLeft(),
							y  - paint_menuH,
							paint_menu_t.getLeft() + paint_menu_t.getWidth(),
							y);
		}else if(y < -20 ){
			if(paint_menu_b.getVisibility() == View.GONE || paint_menu_b.getVisibility() == View.INVISIBLE){
				paint_menu_t.setVisibility(View.INVISIBLE);
				paint_menu_b.setVisibility(View.VISIBLE);
			}
			paint_menu_b.layout(paint_menu_b.getLeft(),
							h + y,
							paint_menu_b.getLeft() + paint_menu_b.getWidth(),
							h + y  + paint_menuH);
		}else{
			paint_menu_t.layout(paint_menu_t.getLeft(),
					-paint_menuH,
