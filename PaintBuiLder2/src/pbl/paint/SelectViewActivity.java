...
public class SelectViewActivity extends Activity implements OnClickListener{
	...
	public void onClick(View v){
		if(v.getTag()=="new"){
			editText = new EditText(SelectViewActivity.this);
			...
			showTDialog(this,"名前を入力してください",editText,new DialogInterface.OnClickListener(){
				public void onClick(DialogInterface dia,int which){
					File f=new File(path+editText.getText().toString()+".jpg");
					if(!f.exists()){
						try {
							new FileOutputStream(path+editText.getText().toString()+".jpg");
							pagerLoader();
							Toast.makeText(SelectViewActivity.this, "作成しました", Toast.LENGTH_SHORT).show();
						} catch (FileNotFoundException e) {
							// TODO 自動生成された catch ブロック
							e.printStackTrace();
						}
					}else{
						Toast.makeText(SelectViewActivity.this, "同じ名前のファイルが存在します",Toast.LENGTH_SHORT).show();
					}
				}
			});
